...
class Admin_Page:
    ...
    def admin_mainframe(self, employee_id):
        ...
        search_button = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.call_search_account_page(self.connection, self.admin_main_window, search_entry.get()),
            relief="flat"
        )
        search_button.place(
            x=66.0,
        ...
        delete_button = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.call_deletion_page(employee_id, self.connection, cursor, self.admin_main_window, search_entry.get()),
            relief="flat"
        )
        delete_button.place(
            x=458.0,
        ...
        edit_button = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda:self.call_add_or_edit_page(self.connection, cursor, self.admin_main_window, search_entry.get(), "Edit User", employee_id),
            relief="flat"
        )
        edit_button.place(
            x=329.0,
        ...
        add_button = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.call_add_or_edit_page(self.connection, cursor, self.admin_main_window, search_entry.get(), "Add User", employee_id),
            relief="flat"
        )
        add_button.place(
            x=196.0,
        ...
        self.session.customer_checkings_view(self.connection, self.admin_main_window, employee_id)
    ...
    def call_add_or_edit_page(self, connection, cursor, window, account_id, session_indicator, employee_id):  # session_indicator(either Add or Edit)
        ...
        if search_account_query == "Account Non-existent":
            ...
            if session_indicator == "Add User":
                while True:
                    if self.proceed_to_session("adding information", account_id) == "Yes":
                        ...
                        if search_account_query == "SELECT * FROM customer_information WHERE customer_id = %s":
                            ...
                            if self.employee.check_account_existence(cursor, "checkings_account", "customer_id", account_id):
                                ...
                                top_level_window = Toplevel()
                                self.admin_edit_add_page.main(account_id, top_level_window, session_indicator, "checkings_account", employee_id)
                                break

                        elif search_account_query == "SELECT * FROM checkings_account WHERE checkings_id = %s":
                            top_level_window = Toplevel()
                            self.admin_edit_add_page.main(account_id, top_level_window, session_indicator, "transactions", employee_id)
                            break
                ...
                while True:
                    if self.proceed_to_session("editing information", account_id) == "Yes":
                        
                        if search_account_query == "SELECT * FROM customer_information WHERE customer_id = %s":
                            top_level_window = Toplevel()
                            self.admin_edit_add_page.main(account_id, top_level_window, session_indicator, "customer_information", employee_id)
                            break
    ...
    def call_deletion_page(self, employee_id, connection, cursor, window, account_id):
        search_account_query = self.employee.search_credentials(connection, account_id)
        confirm_deletion = self.confirm_deletion(window, account_id)
        ...
        while True:
            if confirm_deletion == "Deletion Initiated":
                ...
                (self.deletion_status(True, account_id)) if delete_account == True else (self.deletion_status(False,account_id))
                break
