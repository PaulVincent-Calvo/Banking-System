...
class Edit_Add_Page:
    ...
    def main(self, account_id, toplevel_window, session_indicator, table_involved, employee_id):
        ...
        def edit_from_customer_info():
            self.updating_query(employee_id, self.connection, cursor, "customer_id", account_id, "customer_information", dict_for_customer_info)
            
             
        def edit_from_checkings_account():
            self.updating_query(employee_id, self.connection, cursor,  "checkings_id", account_id, "checkings_account", dict_for_checkings_account)
            
            
        def edit_from_transactions():
            self.updating_query(employee_id, self.connection, cursor, "transactions_id", account_id, "transactions", dict_for_transactions)
        ...
        if session_indicator == "Add User":
            ...
            if table_involved == "checkings_account": # add to checkings_account when customer_id is entered
                def add_into_checkings_account():
                    while True:
                        if self.confirm_session("addition to transactions", account_id) == "Confirmed":
                            ...
                            if account_balance == "" or account_status == "":
                                ...
                                self.add_to_sessionlog(self.connection, employee_id, "Add User", "Checkings Account", values, dict_for_checkings_account)
                ...
                def add_into_transactions():
                    ...
                    if self.employee.check_account_existence(cursor, "checkings_account", "checkings_id", receiving_account_entry.get()) or receiving_account_entry.get() == "" or receiving_account_entry.get() == "None":
                        transactions_id = dict_for_transactions.get("transactions_id")
                        while True:
                            if self.confirm_session("addition to transactions", account_id) == "Confirmed":
                                ...
                                if transaction_type_entry.get() == "" or receiving_account_entry.get() == "" or transaction_date_entry.get() == "" or transaction_amount_entry.get() == "":
                                    ...
                                    values = (transactions_id, account_id, transaction_type_entry.get(), receiving_account_entry.get(), check_date_format(), transaction_amount_entry.get())
                                    cursor.execute(query, values)
                                    self.connection.commit()
                                    messagebox.showinfo("Session Status", f"Transaction Added for Checkings ID: {account_id} with Transactions ID: {transactions_id} ")
                                    print(f"Transaction ID: {transactions_id}, Checkings ID: {account_id}, Transaction Type: {transaction_type_entry.get()}, Receiving Account: {receiving_account_entry.get()}, Date: {transaction_date_entry.get()}, Amount: {account_balance_entry.get()}")
                                    values = list(values)
                                    self.add_to_sessionlog(self.connection, employee_id, "Add User", "Transactions", values, dict_for_transactions)
                                    break
                                
                            else: break
                            
                    else: messagebox.showerror("Session Error", f"Receiving account {receiving_account_entry.get()} not found.")
                    ...
                    while True:
                        if self.confirm_session("addition to transactions", account_id) == "Confirmed":
                            if firstname_entry.get() == "" or lastname_entry.get() == "" or email_entry.get() == "" or annual_income_entry.get() == "" or IDType_entry.get() == "" or occupation_entry.get() == "" or address_entry.get() == "":
                                ...
                                self.add_to_sessionlog(self.connection, employee_id, "Add User", "Customer Information", list(values), dict_for_customer_info)
                                break
    ...
    def updating_query(self, employee_id, connection, cursor, id_column, account_id, table_involved, dict_for_table):
        ...
        try:
            if self.confirm_session("modification", account_id) == "Confirmed":
    ...
    def confirm_session(self, session_type, account_id):
        confirm = messagebox.askquestion("Confirm Session", f"Confirm {session_type} for account {account_id} ?")
        status = "Confirmed" if confirm == 'yes' else "Revoked"
        return status
